...
SELECT
    ...
    CAST
    (
        ISNULL(Federal_Tax, 0.00)
      + ISNULL(SS_Tax, 0.00)
      + ISNULL(Medical_Tax, 0.00)
      + ISNULL(CA_State_Tax, 0.00)
      + ISNULL(CA_SDI_Tax, 0.00)
        AS DECIMAL(18,2)
    ) AS Recalculated_Total_Tax,

    CAST
    (
        ISNULL(Gross_Payment, 0.00)
        -
        (
            ISNULL(Federal_Tax, 0.00)
          + ISNULL(SS_Tax, 0.00)
        ...
        AS DECIMAL(18,2)
    ) AS Recalculated_NetPay,

    CAST
    (
        ABS
        (
            ISNULL(Total_Tax, 0.00)
            -
            (
                ISNULL(Federal_Tax, 0.00)
              + ISNULL(SS_Tax, 0.00)
        ...
        AS DECIMAL(18,2)
    ) AS TotalTax_Difference,

    CAST
    (
        ABS
        (
            ISNULL(Employer_NetPay, 0.00)
            -
            (
                ISNULL(Gross_Payment, 0.00)
                -
        ...
        AS DECIMAL(18,2)
    ) AS NetPay_Difference,

    CASE
        WHEN ISNULL(Gross_Payment, 0.00) <= 0
            THEN 'Invalid Gross Payment'

        WHEN ISNULL(Federal_Tax, 0.00) < 0
          OR ISNULL(SS_Tax, 0.00) < 0
          OR ISNULL(Medical_Tax, 0.00) < 0
          OR ISNULL(CA_State_Tax, 0.00) < 0
          OR ISNULL(CA_SDI_Tax, 0.00) < 0
            THEN 'Negative Tax Amount'
        ...
        (
            ...
            THEN 'Net Pay Mismatch'

        WHEN ISNULL(Payment_Validation, 0) <> 1
            THEN 'Payment Validation Flag Not Passed'
