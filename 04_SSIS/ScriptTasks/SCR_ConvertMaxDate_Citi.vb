    ...
    public partial class ScriptMain : Microsoft.SqlServer.Dts.Tasks.ScriptTask.VSTARTScriptObjectModelBase
    {
        public void Main()
        {
            try
            {
                ...
                object resultObject = Dts.Variables["User::Citi_MaxDate_Obj"].Value;
                string maxDateValue = GetMaxDateFromResultSet(resultObject, fallbackDate);

                Dts.Variables["User::Citi_MaxDate_Str"].Value = maxDateValue;

                bool fireAgain = true;
                Dts.Events.FireInformation(
                    0,
                    "SCR_ConvertMaxDate_Citi",
                    "Citi maximum transaction date captured: " + maxDateValue,
                    string.Empty,
                    0,
                    ref fireAgain
                );
            ...
            catch (Exception ex)
            {
                Dts.Events.FireError(
                    0,
                    "SCR_ConvertMaxDate_Citi",
                    "Failed to convert Citi maximum transaction date. Error: " + ex.Message,
                    string.Empty,
                    0
                );
